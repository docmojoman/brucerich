      ...
      <div class="grid-x grid-margin-x margin-top-80">
        <div class="cell medium-8 medium-offset-2">
          ...
          <form method="POST" action="/admin/books">
            ...
            <label>Header:
              <input name="header[{{ $section->id }}][]" type="text" placeholder="Title" value="{{ $section->header }}">
            </label>
            <label>Description:
              <textarea name="description[{{ $section->id }}][]" id="editor" class="editor" cols="30" rows="10">
                {!! $section->description !!}
              </textarea>
            </label>
            @endforeach
